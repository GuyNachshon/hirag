...
MODEL = config['vllm']['llm']['model']
EMBEDDING_MODEL = config['vllm']['embedding']['model']
VLLM_EMBEDDING_URL = config['vllm']['embedding']['base_url']
VLLM_URL = config['vllm']['llm']['base_url']
...
@wrap_embedding_func_with_attrs(embedding_dim=config['model_params']['vllm_embedding_dim'], max_token_size=config['model_params']['max_token_size'])
...
async def vllm_model_if_cache(
    ...
    openai_async_client = AsyncOpenAI(
        api_key=VLLM_API_KEY, base_url=VLLM_URL
    )
    messages = []
    if system_prompt:
