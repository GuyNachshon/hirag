...
EXT_MAP = {
    ...
    FileCategory.DOCUMENT: {".doc", ".docx"},
    FileCategory.SPREADSHEET: {".xls", ".xlsx", ".csv"},
    FileCategory.PRESENTATION: {".ppt", ".pptx"},
    FileCategory.ZIP: {".zip", ".gzip", ".tar", ".tar.gz", ".gz", ".7zip"}
}


def preprocess_dir(input_dir, output_dir, ocr: DotsOCRParser):
    all_results = list()
    for root, _, files in os.walk(input_dir):
        for file in files:
            ...
            if ext in EXT_MAP[FileCategory.TEXT]:
                shutil.move(file_path, output_dir)
            elif ext in EXT_MAP[FileCategory.PDF] or ext in EXT_MAP[FileCategory.IMAGE]:
                res = ocr.parse_file(file_path)
                all_results.append(res)
            elif ext in EXT_MAP[FileCategory.DOCUMENT]:
                mod_path = f"{root}/{name}.pdf"
                if ext == ".docx":
                    convert(file_path, mod_path)
                    ocr.parse_file(mod_path)
                if ext == ".doc":
                    document = Document()
                    document.LoadFromFile(file_path)
                    document.SaveToFile(mod_path, dFormat.PDF)
                    document.Dispose()
                res = ocr.parse_file(mod_path)
                all_results.append(res)
            elif ext in EXT_MAP[FileCategory.PRESENTATION]:
                mod_path = f"{root}/{name}.pdf"
                document = Presentation()
                document.LoadFromFile(file_path)
                document.SaveToFile(mod_path, pFormat.PDF)
                document.Dispose()
                res = ocr.parse_file(mod_path)
                all_results.append(res)
            else:
                print(f"{ext} currently not supported")
    return all_results
